<template>
    <div class="flex flex-col items-center justify-center bg-gray-100 font-sans relative">
        ...
        <label :class="[state.valid ? 'border-emerald-400' : 'border-blue-400 border-dashed', state.error ? 'border-red-500' : '']" class="mx-auto cursor-pointer flex w-full max-w-lg h-[120px] flex-col items-center rounded-xl border-2 bg-white p-3 text-center"> 
            ...
            <h2 class="mt-2 text-sm font-medium text-gray-700 tracking-wide">{{ description }}</h2>
            ...
            <input :name="type" type="file" class="hidden" :accept="accepted" @change="validateFile" />
        </label>
    </div>
</template>
...
import Spinner from './ui/Spinner.vue';
    ...
    const validateFile = async (e: Event): Promise<void> => {
        state.error = false;
        const file = (e.target as HTMLInputElement).files![0]
        if(props.format == 'image'){
           try {
                await validateId(file);        
           } catch (error) {
            console.error(error)
           }
        }
    ...
    const validateId = async (file: File) => {
        state.loading = true;
        const data = new FormData();
        data.append('id', props.idNumber);
        data.append(props.type == 'id-front' ? 'front' : 'back', file);

        try {
            const res = (props.type == 'id-front' ? await $id4face.validateIdFront(data) : await $id4face.validateIdRear(data))
            if(res) {
                state.valid = true;
                emit('file', file)
            }
